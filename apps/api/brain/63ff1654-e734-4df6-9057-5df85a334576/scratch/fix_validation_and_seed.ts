...
<p style="text-align: justify; font-size: 14px; margin-bottom: 15px;">De una parte, como ARRENDADOR, el propietario legítimo del inmueble, y de otra parte, como ARRENDATARIO, Don/Doña <b>{nombre_cliente}</b>, con correo de contacto <b>{correo_cliente}</b> y número telefónico <b>{telefono_cliente}</b>.</p>
...
<div style="text-align: center; width: 45%;">
  ...
  <p style="font-weight: 700; color: #475569;">Firma de la Propiedad / Agencia</p>
</div>
<div style="text-align: center; width: 45%;">
...
<h3 style="color: #0f172a; border-bottom: 2px solid #e2e8f0; padding-bottom: 6px; margin-top: 25px; margin-bottom: 12px; font-size: 16px; font-weight: 700; text-transform: uppercase;">REUNIDOS</h3>
<p style="text-align: justify; font-size: 14px; margin-bottom: 15px;">De una parte, la Propiedad en calidad de PARTE VENDEDORA, y de otra parte, Don/Doña <b>{nombre_cliente}</b> en calidad de PARTE COMPRADORA, con correo electrónico <b>{correo_cliente}</b> y número de teléfono <b>{telefono_cliente}</b>.</p>
...
<div style="text-align: center; width: 45%;">
  ...
  <p style="margin-bottom: 5px;">___________________________________</p>
  <p style="font-weight: 700; color: #475569;">Firma del Vendedor / Representante</p>
</div>
<div style="text-align: center; width: 45%;">
...
<p style="text-align: justify; font-size: 14px; margin-bottom: 15px;">De una parte, la corporación INMOVA en calidad de mandataria, y de otra parte, Don/Doña <b>{nombre_cliente}</b> en calidad de MANDANTE PROPIETARIO.</p>
...
<p style="text-align: justify; font-size: 14px; margin-bottom: 15px;">Con respecto al inmueble catalogado bajo la referencia <b>{inmueble_referencia}</b>, ubicado en la dirección de <b>{inmueble_direccion}</b>, se formaliza el presente acto de entrega de posesión de llaves.</p>
...
<div style="margin-top: 60px; display: flex; justify-content: space-between; font-size: 14px;">
<div style="text-align: center; width: 45%;">
  <p style="margin-bottom: 45px;">Entrega (Agencia INMOVA):</p>
  <p style="margin-bottom: 5px;">___________________________________</p>
...
<p style="text-align: justify; font-size: 14px; margin-bottom: 15px;">Con respecto a la finalización definitiva de la relación contractual vinculada al inmueble en <b>{inmueble_direccion}</b>, referencia de catálogo <b>{inmueble_referencia}</b>, se efectúa formalmente el retorno del inmueble.</p>
...
<div style="text-align: center; width: 45%;">
  <p style="margin-bottom: 45px;">Recibe a Conformidad (INMOVA):</p>
  <p style="margin-bottom: 5px;">___________________________________</p>
...
async function run() {
  ...
  // 1. Modificar el validator de la colección plantillas para incluir enums en mayúsculas y minúsculas
  console.log('Modifying collection validator for "plantillas"...');
  try {
    await db.command({
      collMod: 'plantillas',
      validator: {
        $jsonSchema: {
          ...
          properties: {
            ...
            tipoContrato: {
              ...
              enum: [
                'Alquiler', 'Arras', 
                'Honorarios del Comprador', 'Honorarios del Vendedor', 
                'Recibo de llaves', 'Devolución de llaves',
                'Honorarios del comprador', 'Honorarios del vendedor' // Retrocompatibilidad
              ],
    ...
    console.log('Collection validator modified successfully!');
  } catch (err: any) {
    console.error('Error modifying validator:', err.message || err);
  }

  // 2. Insertar las plantillas por defecto
  let seededCount = 0;
  for (const dp of defaultPlantillas) {
    const exists = await db.collection('plantillas').findOne({ titulo: dp.titulo });
    if (!exists) {
      const doc = {
        ...dp,
        createdAt: new Date(),
      ...
      console.log(`Already exists: "${dp.titulo}"`);
    }
  }

  console.log(`Seeding complete. Seeded ${seededCount} new templates.`);
